...
export default function ToMakeSandwich() {
  ...
  const editTodo = e => {
    e.stopPropagation()
    setTodos(todos =>
      todos.map(todo => todo.id === e.target.id ? {...todo, editValue: todo.text} : todo)
    )
  }

  const saveEditedTodo = e => {
    setTodos(todos =>
      todos.map(todo => todo.id === e.target.id ? {
        ...todo,
        text: todo.editValue.trim().length === 0 ? todo.text : todo.editValue,
        editValue: todo.editValue.trim().length === 0 ? todo.editValue : null
      } : todo)
    )
  }
  ...
  const cancelEditedTodo = e => {
    setTodos(todos =>
      todos.map(todo => todo.id === e.target.id ? {...todo, editValue: null} : todo)
    )
  }

  const changeTodoEditValue = e => {
    setTodos(todos =>
      todos.map(todo => todo.id === e.target.id ? {...todo, editValue: e.target.value} : todo)
    )
  }
